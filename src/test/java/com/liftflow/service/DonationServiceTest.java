...
class DonationServiceTest {
    ...
    void refundDonation_success_createsRefundTx_andDecreasesJar() {
        ...
        donation.setDonationId(Integer.valueOf(500));
        donation.setUser(user);
        donation.setJar(jar);

        Transaction original = new Transaction();
        original.setTransactionId(Integer.valueOf(100));
        ...
        when(txRepo.findById(Integer.valueOf(100))).thenReturn(Optional.of(original));

        Transaction refund = service.refundDonation(Integer.valueOf(100), new BigDecimal("1000.00"));
    ...
    void addDonation_userNotFound_throws() {
        DonationRequest req = new DonationRequest();
        req.setUserId(Integer.valueOf(999)); req.setJarId(Integer.valueOf(10));
    ...
    void addDonation_jarNotFound_throws() {
        DonationRequest req = new DonationRequest();
        req.setUserId(Integer.valueOf(1)); req.setJarId(Integer.valueOf(999));
        req.setAmount(new BigDecimal("10.00"));
        when(userRepo.findById(Integer.valueOf(1))).thenReturn(Optional.of(new User()));
        when(jarRepo.findById(Integer.valueOf(999))).thenReturn(Optional.empty());
        assertThrows(IllegalArgumentException.class, () -> service.addDonation(req));
    }
    ...
    void refundDonation_exceedsOriginal_throws() {
        Transaction original = new Transaction();
        original.setTransactionId(Integer.valueOf(1));
        original.setTransactionAmount(new BigDecimal("50"));
        when(txRepo.findById(Integer.valueOf(1))).thenReturn(Optional.of(original));
        assertThrows(IllegalArgumentException.class,
                () -> service.refundDonation(Integer.valueOf(1), new BigDecimal("60")));
    }
