...
public class DonationService {
    ...
    public Transaction refundDonation(Integer originalTxId, BigDecimal refundAmount) {
        ...
        // 1) Create refund transaction (self-reference)
        Transaction refund = new Transaction();
        refund.setTransactionAmount(refundAmount.negate()); // необязательно; сумма возврата — как хочешь хранить
        ...
        // 2) Update original (info about refund)
        ...
        // 3) Decrease jar amount
        jar.setCurrentAmount(jar.getCurrentAmount().subtract(refundAmount));
        jar.setUpdatedAt(LocalDateTime.now());
        jarRepo.save(jar);
